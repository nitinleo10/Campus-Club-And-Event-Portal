...
export const createEvent = async (req, res, next) => {
    try {
        ...
        const [userResults] = await connection.promise().query(`SELECT Uid FROM members WHERE Uid = ?`, [userId]);
        if (userResults.length === 0) {
            return next(createCustomError("User not found", 404));
        }

        const uID = userResults[0].Uid;
        const [adminResults] = await connection.promise().query(`SELECT * FROM admin WHERE aid = ?`, [uID]);
        
        if (adminResults.length === 0) {
            return res.status(400).json({ msg: "Unauthorized user" });
        }

        const clubId = adminResults[0].cid;

        // Insert event into the database
        const [result] = await connection.promise().query(
            `INSERT INTO event (Ename, Edate, Edescription, Esummary, Rlink, C_id, Ephoto) VALUES (?, ?, ?, ?, ?, ?, ?)`,
            [name, date, description, summary, link, clubId, photo]
        );
